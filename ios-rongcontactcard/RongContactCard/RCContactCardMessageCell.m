...
- (void)initialize {
    ...
    //头像imageView
    self.portraitView =
    [[RCloudImageView alloc] initWithFrame:CGRectMake(16, 12, 45, 45)];
    ...
    [self.portraitView setPlaceholderImage:[RCCCUtilities imageNamed:@"default_portrait_msg" ofBundle:@"RongCloud.bundle"]];
    ...
    //长按手势
    UILongPressGestureRecognizer *messageLongTap = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(longPressed:)];
...
- (void)setAutoLayout {
    ...
    if (cardMessage) {
        ...
        if(portraitUri.length < 1) {
          ...
          if (userInfo == nil || userInfo.portraitUri.length < 1) {
            if ([[RCIM sharedRCIM].userInfoDataSource respondsToSelector:@selector(getUserInfoWithUserId:completion:)]) {
              [[RCIM sharedRCIM].userInfoDataSource getUserInfoWithUserId:cardMessage.userId completion:^(RCUserInfo *userInfo) {
                dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (MessageDirection_RECEIVE == self.messageDirection) {
      ...
      self.typeLabel.frame = CGRectMake(12, 12+45+12+1, 100, 20);
        ...
        self.bubbleBackgroundView.frame = CGRectMake(
                                                     0, 0, bubbleBackgroundViewSize.width, bubbleBackgroundViewSize.height);
      UIImage *image = [RCKitUtility imageNamed:@"chat_to_bg_white"
                                       ofBundle:@"RongCloud.bundle"];
