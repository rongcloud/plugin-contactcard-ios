...
- (NSArray<RCExtensionPluginItemInfo *> *)getPluginBoardItemInfoList:(RCConversationType)conversationType
  ...
  if (conversationType == ConversationType_PRIVATE ||
    ...
    cardItem.image = [RCKitUtility imageNamed:@"card.png"
    ...
    cardItem.tapBlock = ^(RCChatSessionInputBarControl *chatSessionInputBar){
      ...
      if ([[RCContactCardKit shareInstance].contactVCDelegate respondsToSelector:@selector(needDisplayContactViewController:targetId:)]) {
        [[RCContactCardKit shareInstance].contactVCDelegate needDisplayContactViewController:conversationType targetId:targetId];
      }else {
